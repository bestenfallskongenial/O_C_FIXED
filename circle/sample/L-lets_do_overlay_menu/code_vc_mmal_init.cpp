...
bool            CKernel::initTexturesMMAL         (   )
{
                ...
                if( f_count != 0)
                    { 
#ifdef __LOG_MMAL__ 
                    storeLogHex(   MY_BFR, MY_IDX, "Texture Creation    FAILED");
#endif                   
                    return false;
                    }
#ifdef __LOG_MMAL__             
                storeLogHex(   MY_BFR, MY_IDX, "Texture Creation    SUCCESS");
#endif               
                return true;
}
                                                        ...
                                                        MMAL_Component_Create_Reply& rx)
{
                ...
                if (!sendAndWaitVCHI( m_ServiceHandleMMAL, m_EventMMAL, &tx, sizeof(tx), &rx, sizeof(rx), &rx_len))
                    {
#ifdef __LOG_MMAL__
                    storeLogHex(   MY_BFR, MY_IDX, "MMALsendAndWait FAILED - MSG #", tx.hdr.context );
#endif 
                    return false;
                    }
                                                        ...
                                                        MMAL_Port_Parameter_Set_Reply& rx )
{
            ...
            //  memset(tx.msg.value, 0, sizeof(tx.msg.value));                                          // OLD
                ...
                if (!sendAndWaitVCHI( m_ServiceHandleMMAL, m_EventMMAL, &tx, tx_len, &rx, sizeof(rx), &rx_len ))
                    {
#ifdef __LOG_MMAL__
                    storeLogHex(   MY_BFR, MY_IDX, "MMALsendAndWait FAILED - MSG #", tx.hdr.context );
#endif
                                                        ...
                                                        MMAL_Port_Info_Set_Msg& tx)
{
            //  tx.msg                                            = {};
                ...
                tx.msg.es.video.width                              = MIN_VID_H; //m_ResolutionX;
                tx.msg.es.video.height                             = MAX_VID_H; //m_ResolutionY;
                tx.msg.es.video.crop.x                             = 0;
                tx.msg.es.video.crop.y                             = 0;
                tx.msg.es.video.crop.width                         = MIN_VID_H; //m_ResolutionX;
                tx.msg.es.video.crop.height                        = MAX_VID_H; //m_ResolutionY;
}
